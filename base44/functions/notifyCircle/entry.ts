...
// ────────────────────────────────────────────────────────────────
// notifyCircle
// The member's side of notifications. Sending happens inside the
// functions that cause it (requestCircleJoin, createCirclePost,
// moderateCircle) so a post and its notification cannot drift apart.
// This function owns what a member can change herself.
//
// Payload: { groupId, action, ...args }
//   get_pref                     her notify_pref for this room
//   set_pref                     notifyPref: mine | all | none
//   mark_read                    notificationIds: string[]   (her own only)
// ────────────────────────────────────────────────────────────────
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { email, membership, isApproved, isHost, isAdmin } = ctx;
    const svc = base44.asServiceRole.entities;
    ...
    if (action === "mark_read") {
      const ids = Array.isArray(p.notificationIds) ? p.notificationIds.filter((x: unknown) => typeof x === "string").slice(0, 100) : [];
      let updated = 0;
      for (const id of ids) {
        const rows = await svc.Notification.filter({ id, recipient_email: email });
        const n = Array.isArray(rows) ? rows[0] : null;
        if (n) { await svc.Notification.update(n.id, { isRead: true }).catch(() => {}); updated++; }
      }
