...
export default function ContinueHero({ module, expert, pageProgress, totalPages, moduleIndex, totalModules }) {
  const completedPages = pageProgress || 0;
  const total = totalPages || 1;
  const progressPercent = Math.round((completedPages / total) * 100);
  const remainingMin = module?.durationMinutes ? Math.round(module.durationMinutes * (1 - progressPercent / 100)) : 22;
      ...
      <div className="relative aspect-video bg-gradient-to-br from-[#3D0F1F] via-[#5C1A2E] to-[#3D0F1F] flex items-center justify-center">
        ...
        <div className="absolute top-4 left-5">
          <p className="text-[10px] tracking-[0.2em] text-white/80 font-medium uppercase">
            MODULE {String(moduleIndex || 4).padStart(2, "0")} OF {String(totalModules || 9).padStart(2, "0")}
          </p>
        </div>
        ...
        <div className="flex flex-col md:flex-row md:gap-8">
          ...
          <div className="flex-1 mb-4 md:mb-0">
            ...
            <h2 className="text-xl md:text-2xl text-[#2A1218] leading-snug mb-3" style={{ fontFamily: "Georgia, 'Times New Roman', serif" }}>
              {module?.title || "How Cortisol Disrupts Your Hormones"}
            </h2>
            {/* Expert byline */}
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-full bg-[#5C1A2E] flex items-center justify-center flex-shrink-0">
                {expert?.profile_picture ? (
                  ...
                  <span className="text-white text-xs font-medium">{expert?.name?.[0] || "E"}</span>
                )}
              </div>
              <div>
                <p className="text-sm text-[#2A1218] font-medium">{expert?.name || "Expert"}</p>
                <p className="text-[10px] tracking-[0.15em] text-[#6B6168] uppercase">{expert?.title || "Specialist"}</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 md:max-w-[280px]">
            ...
            <div className="bg-[#F5E6E8] rounded-lg p-3 mb-3">
              <p className="text-[10px] tracking-[0.15em] text-[#5C1A2E] font-medium uppercase mb-2">
                WORKBOOK · PAGE {completedPages} OF {total} · {progressPercent}%
              </p>
