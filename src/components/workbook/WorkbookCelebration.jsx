...
export default function FeminineWorkbook() {
  ...
  // Synchronous mirror of responseId plus a single in-flight create promise.
  // Every save path routes creation through ensureResponseId so rapid edits
  // can never create more than one WorkbookResponse row for this workbook.
  ...
  const ensureResponseId = useCallback(async () => {
    ...
    if (!createPromiseRef.current) {
      createPromiseRef.current = base44.entities.WorkbookResponse
        .create({ workbook_id: WORKBOOK_ID, answers: {} })
        .then((created) => {
  ...
  const jumpTo = useCallback(async (idx) => {
    ...
    if (sId) {
      ...
      saveTimer.current = setTimeout(async () => {
        try {
          const id = await ensureResponseId();
          await base44.entities.WorkbookResponse.update(id, { last_section_id: sId });
          setLastSaved(new Date());
        } catch (e) {
  ...
  const handleComplete = async () => {
    ...
    try {
      const id = await ensureResponseId();
      await base44.entities.WorkbookResponse.update(id, { answers, ...payload });
    } catch (e) {
...
function FemSectionContent({ section, answers, onAnswerChange, allSections, isLastSection, isComplete, onComplete, onBackToDashboard }) {
  return (
    <div>
      {section.part_label && (
        <p style={{
